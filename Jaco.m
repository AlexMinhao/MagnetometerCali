function [delt_F,Loss]= Jaco(qd,m,sh)
    syms s11 s12 s13 s21 s22 s23 s31 s32 s33 h1 h2 h3 
    ...
    M1 = inv(S)*(M1-H);
    M1 = [0;M1]'; 
    Mc =  mult(mult(Q1,M1),q_inv(Q1)); % 1 x 4
    Mc =  Mc(:,2:4);
    Mc = S * Mc' + H;
    l1 = abs([Mc(1);Mc(2);Mc(3)] - M2);
    
%     M2 = inv(S)*(M2-H);
%     M2 = [0;M2]'; 
%     Mc =  mult(mult(Q2,M2),q_inv(Q2)); % 1 x 4
%     l2 = abs([Mc(1);Mc(2);Mc(3)] - M3);
    
    M3 = inv(S)*(M3-H);
    M3 = [0;M3]'; 
    Mc =  mult(mult(Q2,M3),q_inv(Q2)); % 1 x 4
    Mc =  Mc(:,2:4);
    Mc = S * Mc' + H;
    l2 = abs([Mc(1);Mc(2);Mc(3)] - M4);
%     
%     M4 = inv(S)*(M4-H);
%     M4 = [0;M4]'; 
%     Mc =  mult(mult(Q4,M4),q_inv(Q4)); % 1 x 4
%     l4 = abs([Mc(1);Mc(2);Mc(3)] - M5);
    
    M5 = inv(S)*(M5-H);
    M5 = [0;M5]'; 
    Mc =  mult(mult(Q3,M5),q_inv(Q3)); 
    Mc =  Mc(:,2:4);
    Mc = S * Mc' + H;
    l3 = abs([Mc(1);Mc(2);Mc(3)] - M6);
    
%     M6 = inv(S)*(M6-H);
%     M6 = [0;M6]'; 
%     Mc =  mult(mult(Q6,M6),q_inv(Q6)); 
%     l6 = abs([Mc(1);Mc(2);Mc(3)] - M7);
    
    M7 = inv(S)*(M7-H);
    M7 = [0;M7]'; 
    Mc =  mult(mult(Q4,M7),q_inv(Q4)); 
    Mc =  Mc(:,2:4);
    Mc = S * Mc' + H;
    l4 = abs([Mc(1);Mc(2);Mc(3)] - M8);
    
%     M8 = inv(S)*(M8-H);
%     M8 = [0;M8]'; 
%     Mc =  mult(mult(Q8,M8),q_inv(Q8)); 
%     l8 = abs([Mc(1);Mc(2);Mc(3)] - M9);
    
    M9 = inv(S)*(M9-H);
    M9 = [0;M9]'; 
    Mc =  mult(mult(Q5,M9),q_inv(Q5)); 
    Mc =  Mc(:,2:4);
    Mc = S * Mc' + H;
    l5 = abs([Mc(1);Mc(2);Mc(3)] - M10);
    
%     M10 = inv(S)*(M10-H);
%     M10 = [0;M10]'; 
%     Mc =  mult(mult(Q10,M10),q_inv(Q10)); 
%     l10 = abs([Mc(1);Mc(2);Mc(3)] - M11);
    
    M11 = inv(S)*(M11-H);
    M11 = [0;M11]'; 
    Mc =  mult(mult(Q6,M11),q_inv(Q6)); 
    Mc =  Mc(:,2:4);
    Mc = S * Mc' + H;
    l6 = abs([Mc(1);Mc(2);Mc(3)] - M12);
    
%     M12 = inv(S)*(M12-H);
%     M12 = [0;M12]'; 
%     Mc =  mult(mult(Q12,M12),q_inv(Q12)); 
%     l12 = abs([Mc(1);Mc(2);Mc(3)] - M13);
    
    M13 = inv(S)*(M13-H);
    M13 = [0;M13]'; 
    Mc =  mult(mult(Q7,M13),q_inv(Q7)); 
    Mc =  Mc(:,2:4);
    Mc = S * Mc' + H;
    l7 = abs([Mc(1);Mc(2);Mc(3)] - M14);
    
%     M14 = inv(S)*(M14-H);
%     M14 = [0;M14]'; 
%     Mc =  mult(mult(Q14,M14),q_inv(Q14)); 
%     l14 = abs([Mc(1);Mc(2);Mc(3)] - M15);
    
    M15 = inv(S)*(M15-H);
    M15 = [0;M15]'; 
    Mc =  mult(mult(Q8,M15),q_inv(Q8)); 
    Mc =  Mc(:,2:4);
    Mc = S * Mc' + H;
    l8 = abs([Mc(1);Mc(2);Mc(3)] - M16);
    
%     M16 = inv(S)*(M16-H);
%     M16 = [0;M16]'; 
%     Mc =  mult(mult(Q16,M16),q_inv(Q16)); 
%     l16 = abs([Mc(1);Mc(2);Mc(3)] - M17);
    
    M17 = inv(S)*(M17-H);
    M17 = [0;M17]'; 
    Mc =  mult(mult(Q9,M17),q_inv(Q9)); 
    Mc =  Mc(:,2:4);
    Mc = S * Mc' + H;
    l9 = abs([Mc(1);Mc(2);Mc(3)] - M18);
    
%     M18 = inv(S)*(M18-H);
%     M18 = [0;M18]'; 
%     Mc =  mult(mult(Q18,M18),q_inv(Q18)); 
%     l18 = abs([Mc(1);Mc(2);Mc(3)] - M19);
    
    M19 = inv(S)*(M19-H);
    M19 = [0;M19]'; 
    Mc =  mult(mult(Q10,M19),q_inv(Q10)); 
    Mc =  Mc(:,2:4);
    Mc = S * Mc' + H;
    l10 = abs([Mc(1);Mc(2);Mc(3)] - M20);
    

    L = l1+l2+l3+l4+l5+l6+l7+l8+l9+l10; % 3 x 1

    %L = L * L';                       % 3 x 3
    J=jacobian([L(1);L(2);L(3)],[s11 s12 s13 s21 s22 s23 s31 s32 s33 h1 h2 h3]); % 3 x 12
    delt_f = J'* L;
